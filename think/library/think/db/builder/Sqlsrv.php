...
{
    protected $selectSql       = 'SELECT T1.* FROM (SELECT think.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING%) AS think) AS T1 %LIMIT%%COMMENT%';
